...
calc_stat_balance <- function(hp, atk, def, spatk, spdef, spd) {
  ...
  # Exponential decay function - adjusted for higher baseline
  # Perfect balance (CV=0) = 20 points
  # High variance (CV=1.0) ≈ 7.4 points (increased from 5)
  # Formula: 20 * exp(-2 * cv) with floor at 5 (gentler decay)
  balance_score <- 20 * exp(-2 * cv)
...
calc_accessibility_score <- function(category_label) {
  # Map categories to numeric difficulty (0=easiest to obtain, 2=hardest)
  difficulty <- case_when(
    category_label == "Regular" ~ 0.3,        # Easy to get, ~9.8 points
    category_label == "Pseudo-Legendary" ~ 1.0,  # Moderate effort, ~5 points
    category_label == "Legendary" ~ 1.5,      # Hard to get, ~2.7 points
    category_label == "Mythical" ~ 2.0,       # Very hard, ~1.2 points
    TRUE ~ 1.0  # Default to medium difficulty
  )
  
  # Sigmoid function for smooth transition
  # Regular (0.3) → ~9.8 points, Legendary (1.5) → ~2.7 points
  # Formula: 10 / (1 + exp(1.8 * (difficulty - 0.8)))
  score <- 10 / (1 + exp(1.8 * (difficulty - 0.8)))
...
pokemon_rated <- pokemon_df |>
  ...
  mutate(
    ...
    # Tier Assignment (adjusted thresholds for realistic distribution)
    tier = case_when(
      total_rating >= 75 ~ "S",  # Top ~1-2% (降低from 85)
      total_rating >= 65 ~ "A",  # Strong Pokemon (~10-15%)
      total_rating >= 55 ~ "B",  # Above average (~30-40%)
      total_rating >= 45 ~ "C",  # Average (~30%)
      total_rating >= 35 ~ "D",  # Below average (~15%)
      TRUE ~ "F"                 # Weak Pokemon (~5%)
    )
  ) |>
  ungroup()
...
balance_viz <- tibble(
  ...
  Score = sapply(cv_range, function(cv) {
    20 * exp(-2 * cv)  # Updated to match new formula
  })
)
...
write_csv(pokemon_rated, here("data", "pokemon_rated.csv"))

# Create summary statistics
rating_summary <- pokemon_rated |>
  summarise(
    total_pokemon = n(),
    avg_rating = mean(total_rating),
    median_rating = median(total_rating),
    min_rating = min(total_rating),
    max_rating = max(total_rating),
    ...
    avg_type_score = mean(type_score),
    avg_balance_score = mean(balance_score),
    avg_access_score = mean(access_score)
  )

write_csv(rating_summary, here("data", "rating_summary.csv"))

cat("\n=== RATING SUMMARY ===\n")
cat(sprintf("Total Pokemon: %d\n", rating_summary$total_pokemon))
cat(sprintf("Average Rating: %.1f\n", rating_summary$avg_rating))
cat(sprintf("Rating Range: %.1f - %.1f\n", rating_summary$min_rating, rating_summary$max_rating))
cat(sprintf("\nTier Distribution:\n"))
...
cat(sprintf("  Type Score: %.1f / 30\n", rating_summary$avg_type_score))
cat(sprintf("  Balance Score: %.1f / 20\n", rating_summary$avg_balance_score))
cat(sprintf("  Accessibility Score: %.1f / 10\n", rating_summary$avg_access_score))

cat("\n✓ Results exported to:\n")
cat("  - data/pokemon_rated.csv\n")
cat("  - data/rating_summary.csv\n")
```
