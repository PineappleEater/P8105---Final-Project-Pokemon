...
#### Exported CSV Files:
`pokemon_data.csv`: Complete dataset with all 1,219 Pokémon (all evolution stages, all forms).  
`pokemon_data_final_evolutions.csv`: **Recommended for analysis** - Contains only 743 fully evolved Pokémon to eliminate downward bias from unevolved forms.

#### In-Memory R Objects:
**Primary Datasets:**  
`pokemon_df`: Complete dataset with all 9 generations (1,219 Pokémon including all evolution stages).  
`pokemon_evolved_df`: **Final evolutions only** (743 Pokémon) - Use this to avoid statistical bias.

**Special Category Subsets (All Evolution Stages):**  
`pokemon_legend`: Legendary Pokémon subset.  
`pokemon_myth`: Mythical Pokémon subset.  
`pokemon_para`: Paradox Pokémon subset.  
`pokemon_ub`: Ultra Beast Pokémon subset.

**Auxiliary Objects:**  
`pokemon_evolved_n`: Lookup table containing dex numbers of all final evolution forms.  
`legend_evolved_df`: Final evolution Legendary Pokémon (in-memory only).  
`myth_evolved_df`: Final evolution Mythical Pokémon (in-memory only)  
...
write_csv(pokemon_evolved_df, "data/pokemon_evolved_df.csv")
