...
```{r load-data}
# Load enriched dataset with features
pokemon_df <- read_csv("data/pokemon_data_enriched.csv")
...
# Compile all binary classification results
rf_tuned_cm <- confusionMatrix(predict(rf_tuned_model, test_data), 
                               test_data$is_legendary, 
                               positive = "Legendary")

final_metrics <- tibble(
  Model = c("Logistic", "Tree", "Random Forest", 
            "RF Tuned", "XGBoost"),
  Accuracy = c(
    logistic_cm$overall["Accuracy"],
    tree_cm$overall["Accuracy"],
  ...
  AUC = c(
    ...
    auc(roc_tree),
    auc(roc_rf),
    auc(roc(test_data$is_legendary, 
           predict(rf_tuned_model, test_data, type = "prob")$Legendary)),
    auc(roc(test_data$is_legendary, xgb_pred$Legendary))
  )
) |>
  mutate(across(where(is.numeric), ~round(.x, 3)))
...
final_metrics |>
  ...
  labs(title = "Comprehensive Model Performance Comparison",
       subtitle = "All Metrics Across 7 Models",
       y = "Score") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
saveRDS(xgb_model, "models/legendary_classifier_xgboost.rds")
saveRDS(multi_rf_model, "models/multiclass_classifier_rf.rds")
...
- ✅ Comprehensive hyperparameter tuning (Random Forest & XGBoost)
- ✅ Grid search with cross-validation
- ✅ Multi-class classification for all Pokemon categories
- ✅ Performance comparison across 5 binary models + 1 multi-class model
